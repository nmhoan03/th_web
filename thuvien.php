...
function taodonhang($name, $address, $tel,$email,$total,$pttt){
    $conn=ketnoidb();
        $sql = "INSERT INTO bill (name, address, tel, email, total, pttt,trangthai)
        VALUES ('$name', '$address', '$tel', '$email', '$total', '$pttt','Chưa Duyệt')";
        // use exec() because no results are returned
        $conn->exec($sql);
...
function getOrdersByUser($email){  
    $conn = ketnoidb();
    $sql = "SELECT * FROM bill WHERE email = :email ORDER BY id DESC";
