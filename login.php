...
{
    global $conn;
    $mkMD5 = $mk; // Bạn có thể muốn mã hóa mật khẩu ở đây
    $sql = "SELECT * FROM `taikhoan` WHERE `tendangnhap`='$tendn' AND `matkhau`='$mkMD5'";
    $result = mysqli_query($conn, $sql);

    if ($result && mysqli_num_rows($result) > 0) {
        return mysqli_fetch_assoc($result); // Trả về thông tin người dùng
    } else {
...
if (isset($_POST['ok'])) {
    ...
    $userData = checklogin($tendangnhap, $matkhau);
    if ($userData) {
        // Lưu từng session riêng biệt
        $_SESSION['user'] = $tendangnhap;
        $_SESSION['user_permission'] = $userData['quyen'];
